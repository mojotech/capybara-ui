Feature: .list macro
  ...
  Although we can use the `widget` macro to declare a list, Dill provides a slightly more convenient way to do it, using the `list` macro.
  ...
  Scenario: Using the default selector
    ...
    We can also pass the widget name only, and `list` will use Dill::List's default selector (`ul`) and item selector (`li`).
